...
#write.csv(df_diagnosis, file=paste(project_path, "/data/dataframes/df_diagnosis_1y.csv", sep=''))
...
pat_icds_labelled <- left_join(x= ICD_Groups, y= df_diagnosis, by = c("ICD10" = "source_concept_code")) %>% distinct()
...
```{r}
pat_icds_labelled <- left_join(x= ICD_Singles, df_diagnosis, by = c("ICD10" = "source_concept_code")) %>%  distinct() 
