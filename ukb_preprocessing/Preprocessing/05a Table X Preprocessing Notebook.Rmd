...
# 
# df_pdff <- fread("processed/UKB_MRI_PDFF.csv")
# 
# head(df_pdff)
```
...
#Setup empty dfs for modalities
df_blood <- df_eid
df_metabolomics <- df_eid
df_snp <- df_eid

```
```{r}

# Sensitivity analysis, loading data with 1 year future info into df_diagnosis
df_diagnosis <- fread(file.path(project_path, "data/dataframes/df_diagnosis_1y.csv")) %>% check_and_remove_withdrawals(df_withdrawals)
```
...
```{r}
setwd(sharepoint_ukb)
df_pdff <- fread("raw/ukb52200.csv", select=c("eid", "40061-1.0", "40061-2.0")) %>% check_and_remove_withdrawals(df_withdrawals)
```
#### Optional: Blood data instance 1
...
# df_blood_i1 <- fread("raw/ukb52200.csv", select=c("eid", "30160-1.0","30220-1.0","30150-1.0","30210-1.0","30030-1.0","30020-1.0","30300-1.0","30290-1.0","30280-1.0","30120-1.0","30180-1.0","30050-1.0","30060-1.0","30040-1.0","30100-1.0","30260-1.0","30270-1.0","30130-1.0","30190-1.0","30140-1.0","30200-1.0","30170-1.0","30230-1.0","30080-1.0","30090-1.0","30110-1.0","30010-1.0","30070-1.0","30250-1.0","30240-1.0","30000-1.0","30620-1.0","30600-1.0","30610-1.0","30630-1.0","30640-1.0","30650-1.0","30710-1.0","30680-1.0","30690-1.0","30700-1.0","30720-1.0","30660-1.0","30730-1.0","30740-1.0","30750-1.0","30760-1.0","30770-1.0","30780-1.0","30790-1.0","30800-1.0","30810-1.0","30820-1.0","30830-1.0","30850-1.0","30840-1.0","30860-1.0","30870-1.0","30880-1.0","30670-1.0","30890-1.0")) %>% check_and_remove_withdrawals(df_withdrawals)
# 
# setwd(sharepoint_ukb)
# write.csv(df_blood_i1, file="extracted/UKB_Patients_blood_instance_1.csv", row.names=FALSE)    
# 
# #setwd(sharepoint_ukb)
```
...
na_blood_col$feature <- rownames(na_blood_col)  # Move rownames to a new column
na_blood_col <- na_blood_col[, c("feature", setdiff(names(na_blood_col), "feature"))]  # Reorder: feature first
write.xlsx(na_blood_col, file=paste(project_path, "/supplement/NA_count_df_blood.xlsx", sep=''))
...
if (na_mode == "impute") {
  ...
  df_blood <- omit.NA(df_blood, 1)   # Delete rows with > 20 NAs per Row
...
```{r}
# #Optional: Specific df_blood consisting only of TOP15 params
# row_subset <- "par"
# subsets <- read_excel(paste0(project_path, "/data/reduce_columns_", row_subset, ".xlsx"))
# top15 <- subsets$Feature[subsets$top15 == 1]
# top15 <- top15[!is.na(top15) & top15 != ""]
# df_blood <- df_blood %>%
#   select(eid, any_of(top15))
```



```{r}
df_blood <- adjust_outliers(df_blood)
hist(df_blood$`Aspartate aminotransferase`)
```
...
files <- grep("\\.raw$", files, value = TRUE)
vec <- sort(
  unlist(
    lapply(
      strsplit(files, "_"),
...
setwd(sharepoint_ukb)
for (i in files) {  #Loop through each SNP
  print(i)  #control if loop is working
  snp <- data.frame(read.csv(paste(project_path, "/genetics/", i, sep=""), sep=""))  %>% #import snp, change this name according to your file names!
...
write.csv(df_snp, file=file.path(project_path, "data/dataframes/df_snp_raw.csv"))  
...
#export df_snp
write.csv(df_snp, file=file.path(project_path, "data/dataframes/df_snp.csv"))  
...
merge_dataframes <- function(include_metabolomics = FALSE, filter_par = FALSE, normalize = FALSE) {
        ...
        if (reduce_model) {
          df_merged <- df_merged %>% select(-any_of(vec_remove_columns)) # Removing the columns not necessary
        }
...
process_and_save <- function(df_x, subset_name) {
  ...
  # Export df_all, one large df with all the raw data across all layers (for future reproducibility, visuals etc)
  write.csv(df_x_temp, file=paste(project_path, "/data/", today, "/X_", tolower(layer), "_", col_subset, "_", subset_name, raw, ".csv", sep=""), row.names=FALSE)
...
Execution of normalized merge/process_and save (always execute both
rows! If not, the global variable "row_subset" will not be set
correctly)

### Here, we have to merge X and y for once, to make sure that

a)  for X, all patients with DOI before ukb visit are excluded
b)  for y, that all patients with missing data are excluded from y as
    well
...
today <- format(Sys.Date(), "%d_%m_%Y")
write.csv(df_columngroups, paste(project_path, "/data/", today, "/columngroups.csv", sep=""), row.names=FALSE, quote = FALSE)
...
df_x_all_raw <- merge_dataframes(normalize=FALSE) #Merge all dfs (icd, blood etc)
...
#Check cause of missing data (merge df_y with every single df to check
how many cases are lost) Does not work yet
