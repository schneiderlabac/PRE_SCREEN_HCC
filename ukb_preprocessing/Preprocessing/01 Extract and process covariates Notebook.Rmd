...
#covariate_string <- cat(covariate_index$datafield2, sep="", file="covariate_index.txt") #erstellt .txt datei im Working directory. Diese kann kopiert werden und in die fread-Funktion nach der eid eingefÃ¼gt werden
...
if (!file.exists(file.path(project_path,"data/dataframes/covariates.csv"))){ # if covars not already extracted from the big UKB dataset this will run or you copy the base covariates from the ukb/extracted
  cols=c("eid",read_excel(master_table, sheet = "All covariates")$datafield1)
    covariates<- fread("raw/ukb52200.csv", select=cols)
  write.csv(covariates, file=file.path(project_path,"data/dataframes/covariates.csv"), row.names=FALSE)
}
...
# Create a df for alcohol related columns
          ...
          #Zuerst berechne ich fÃ¼r wöchentlichen Konsum g/d Alcohol 
          # * 400 entspricht Volumen des Glases, * 5/100 enstpricht Alkoholgehalt in prozent, 
          #* 0.8 Aufnahmeanteil im Körper, / 7 fÃ¼r Tage der Woche
          ...
          #Dann berechne ich fÃ¼r monatlichen Konsum g/d Alcohol
...
#c) Family diabetes (labelled with code 9): 0= No, 1= Yes, 2 = Unknown (-> made 0)
          ...
          # bowel cancer
          df_covariates$Family_Bowel_Cancer <- ifelse(df_covariates$`Illnesses of father` == 4 | df_covariates$`Illnesses of mother` == 4 | df_covariates$`Illnesses of siblings` == 4, 1, 0)
          df_covariates$Family_Bowel_Cancer[is.na(df_covariates$Family_Bowel_Cancer)] <- 0
          df_covariates$Family_Bowel_Cancer <- as.factor (df_covariates$Family_Bowel_Cancer)
         
          # brest cancer
          df_covariates$Family_Breast_Cancer <- ifelse(df_covariates$`Illnesses of father` == 5 | df_covariates$`Illnesses of mother` == 5 | df_covariates$`Illnesses of siblings` == 5, 1, 0)
          df_covariates$Family_Breast_Cancer[is.na(df_covariates$Family_Breast_Cancer)] <- 0
          df_covariates$Family_Breast_Cancer <- as.factor (df_covariates$Family_Breast_Cancer)
          
          df_covariates$Family_Prostate_Cancer <- ifelse(df_covariates$`Illnesses of father` == 13 | df_covariates$`Illnesses of mother` == 13 | df_covariates$`Illnesses of siblings` == 13, 1, 0)
          df_covariates$Family_Prostate_Cancer[is.na(df_covariates$Family_Prostate_Cancer)] <- 0
          df_covariates$Family_Prostate_Cancer <- as.factor (df_covariates$Family_Prostate_Cancer)
