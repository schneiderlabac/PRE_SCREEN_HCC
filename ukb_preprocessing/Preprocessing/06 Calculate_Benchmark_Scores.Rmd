...
df_withdrawals <- fread("raw/withdrawals.csv")
...
The formula for FIB-4 is: Age ([yr] x AST [U/L]) / ((PLT [10(9)/L]) x sqrt(ALT [U/L])).
...
df_fib4 <- df_fib4 %>%
  mutate(
    FIB4_orig = (AGE * AST) / (PLT * sqrt(ALT))
  )
...
```{r}
# Access relevant columns and impute NAs
df_liverRisk <- df_blood %>% 
  select(c("eid", "Glucose", "Cholesterol", "Aspartate aminotransferase", 
          "Alanine aminotransferase", "Gamma glutamyltransferase", "Platelet count")) %>%
  rename(
    glucose = `Glucose`,
...
df_liverRisk <- as.data.frame(df_liverRisk)

# Impute missing values
df_liverRisk[,2:7] <- mean.impute(df_liverRisk[,2:7])

# Prepare gender/sex variable (0 = female, 1 = male)
df_liverRisk$gender <- ifelse(df_liverRisk$SEX == "Female", 0, 1)
df_liverRisk$gender <- as.numeric(df_liverRisk$gender)

# Calculate LiverRisk score (original)
df_liverRisk$LiverRisk_orig <- 3.75 + 
                           0.265 * df_liverRisk$gender + 
                           0.0147 * df_liverRisk$AGE + 
                           0.47 * df_liverRisk$glucose - 
                           0.357 * df_liverRisk$cholesterol + 
                           0.021 * df_liverRisk$AST + 
                           0.00178 * df_liverRisk$ALT + 
                           0.0164 * df_liverRisk$GGT - 
                           0.00495 * df_liverRisk$platelets

# Apply min-max normalization to scale between 0 and 1
df_liverRisk <- df_liverRisk %>%
  mutate(
    LiverRisk = (LiverRisk_orig - min(LiverRisk_orig, na.rm = TRUE)) / 
                (max(LiverRisk_orig, na.rm = TRUE) - min(LiverRisk_orig, na.rm = TRUE))
  )

# Keep only eid, LiverRisk and gender
df_liverRisk <- df_liverRisk %>% 
  select(c("eid", "LiverRisk", "gender"))
  
# Summary statistics
summary(df_liverRisk$LiverRisk)
```
```{r}
...
# Add 1 year cirrhosis info as marker
```{r}
df_cirrhosis_1y <- read.csv(paste0(project_path, "/data/dataframes/df_diagnosis_1y.csv")) %>%
  select(c("eid", "Liver.cirrhosis")) %>%
  rename("Cirrhosis" = "Liver.cirrhosis")
...
df_benchmark <- df_amap %>%
  ...
  inner_join(df_liverRisk %>% select(eid, LiverRisk), by = 'eid') %>%
  inner_join(df_cirrhosis_1y %>% select(eid, Cirrhosis), by = 'eid')
...
```{r}
df <- read.csv(file=paste(project_path, "/data/dataframes/df_benchmark.csv", sep=''))
```
...
write.csv(df, file=paste(project_path, "/data/dataframes/df_benchmark.csv", sep=''))
```
...
@Louis if you see this you successfully cloned this script as of 30/05/25
